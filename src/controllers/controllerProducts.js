...
export async function controllerPostSendProduct(req, res) {
  ...
  await transporter.sendMail({
    ...
    to: user.email,
    subject: 'Compra realizada',
    html: `<h3>Nueva compra</h3><h4>Felicidades por tu nueva compra! A continuación te dejo los datos del producto y el link para acceder al pdf completo</h4><h4>Producto: ${product.title}</h4><h4>Precio: ${product.price}</h4><button style='padding:7px; background-color: #1cb85f; color: #fff; border-radius:5px'><a style='text-decoration:none; color: #fff;' href='${product.pdf}'>Producto digital</a></button>`
  })
